...
\end{align}
$${#eq-linear-cef}
where $\beta \equiv [\beta_1, \ldots, \beta_K]'$ is the vector of **coefficients**.

However, since our focus is on using the linear regression model to estimate the CEF, it is natural to treat @eq-linear-cef as the defining assumption of the model.

:::{.assumption-box #assump-linreg2}
...
As in @eq-cef-err, we can *define* the **linear regression error** as

$$
e_i \equiv Y_i - X_i' \beta.
$${#eq-linreg-err}

We can now recover @eq-linearity by simply rearranging the above definition. Additionally, the *assumption* in @eq-linreg-cef and the *definition* in @eq-linreg-err together imply that the previously discussed properties of the CEF error $e$ all hold for the linear regression error $e_i$. Specifically, $e_i$ is **mean-independent**, has **mean-zero**, and is **orthogonal** and **uncorrelated** to any function of $X_i$. The proofs are omitted because they are the same as before (see @eq-mean-ind, @eq-mean-zero, @eq-ortho, @eq-uncorr).
...
# ---- Fit models (linear in parameters, no intercept) ----
m_deg1  <- lm(y ~ x1 - 1, data = train)
m_deg4  <- lm(y ~ poly(x1, 4, raw = TRUE) - 1, data = train)
m_exp   <- lm(y ~ x1 + exp(x1) - 1, data = train)  # <— now includes both x and exp(x)
...
df_pred <- tibble(
  ...
  `x + exp(X)` = predict(m_exp, newdata = tibble(x1 = x_grid))
) |>
