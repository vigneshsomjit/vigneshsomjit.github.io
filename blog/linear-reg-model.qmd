...
$$
\mathbb{E}[e \mid \boldsymbol{X}_1 \ldots , \boldsymbol{X}_n] = 0,
...
It is good practice to carefully think through what is being assumed in any given model. The linearity assumption in @eq-linearity and @eq-linreg-matrix states that (i) the functional form of the relationship between the outcome and predictors is linear in the parameters $\boldsymbol{\beta}=(\beta_1, \ldots, \beta_k)$, and (ii) the error $e$ is **additive**. 
...
1. The unconditional mean of the error term is zero:
$$
\mathbb{E}[e_i] = \mathbb{E}[\mathbb{E}[e_i \mid \mathbf{X}]] = 0 \quad \forall \, i,
...
2. The predictors are **orthogonal** to the error term for *all observations*: 
$$
\begin{align}
\mathbb{E}[X_{jk}e_i] 
\overset{(a)}{=} \mathbb{E}[\mathbb{E}(X_{jk}e_i \mid X_{jk})] 
\overset{(b)}{=} \mathbb{E}[X_{jk} \mathbb{E}[e_i \mid X_{jk}]]  
\overset{(c)}{=} 0 \quad \forall \, i,j,k,
\end{align}
$${#eq-orthogonality}
...
3. The error term $e_i$ is uncorrelated with the predictor variables of *all observations*:
$$
\text{Cov}(e_i, X_{jk}) 
\overset{(a)}{=} \mathbb{E}[e_iX_{jk}] - \mathbb{E}[e_i]\mathbb{E}[x_{jk}]
...
It is important to note that the random sampling assumption relaxes implications $(2)$ and $(3)$ above. Specifically, we have that 
$$
